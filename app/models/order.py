...
class Order(Base):
    ...
    pay_channel: Mapped[int | None] = mapped_column(nullable=True, comment="支付渠道: 1=微信, 2=支付宝, 3=接口调用")
    client_order_no: Mapped[str | None] = mapped_column(String(64), nullable=True, comment="客户侧订单号, 接口下单来源")
...
class OrderItem(Base):
    ...
    exchanged_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True, comment="兑换时间, NULL=未兑换")
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now(), onupdate=func.now())
